...
export const ExploreScreen: React.FC<ExploreScreenProps> = ({
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (val.trim().length > 0) {
      const terms = val.toLowerCase().split(' ').filter(Boolean);
      const filtered = medicines.filter(m => {
        const searchText = `${m.brandName} ${m.genericName} ${m.activeChemical}`.toLowerCase();
    ...
    <div className="pb-24 space-y-4 px-4 pt-3 max-w-2xl mx-auto">
      ...
      <div className="bg-sky-50/80 border border-sky-100/90 rounded-2xl p-4 sm:p-5 shadow-xs">
        ...
        <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight leading-tight">
          {t.explore.heroTitle.replace('.', '')} <span className="text-sky-600">{t.explore.heroTitleHighlight}</span>.
        </h1>
